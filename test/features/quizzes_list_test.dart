...
StudentAssessment _quiz(String id) => StudentAssessment(
      id: id,
      title: 'Quiz $id',
      category: 'quiz',
      type: 'questions',
...
Paginated<StudentAssessment> _page({
  ...
  int items = 2,
    ...
    Paginated<StudentAssessment>(
      items: [for (var i = 0; i < items; i++) _quiz('$page-$i')],
...
/// The Quizzes screen is the assignments endpoint with `category=quiz` pinned:
/// drop that one parameter and the backend does not return an empty list, it
/// returns *assignments*. That, and the `??`-swallows-null trap in `copyWith`,
...
void main() {
  ...
  setUp(() {
    ...
    cubit = QuizzesCubit(listAll: listAll, listCourses: listCourses);
  });

  tearDown(() => cubit.close());

  group('category', () {
    test('is pinned to quiz on every request the screen makes', () async {
      await cubit.load();
      await cubit.setSearch('arrays');
      await cubit.setFilters(courseId: 'course-1', status: 'submitted');

      // Without this the screen silently lists assignments instead — the
      // failure looks like wrong data, not like a missing parameter.
      expect(
