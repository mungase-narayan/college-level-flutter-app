...
/// first. The same nineteen destinations are presented here as an inset-grouped
/// glass sheet, which is how iOS actually surfaces a long secondary menu.
///
/// Reads [StudentNav.groups] directly, so the two platforms cannot drift on what
/// the menu contains or in what order — and `StudentDrawer` stays untouched for
/// Android.
Future<void> showGlassMenuSheet(
  ...
  final destination = await showModalBottomSheet<String>(
    ...
    builder: (context) => _GlassMenuSheet(currentPath: currentPath),
  );
...
class _GlassMenuSheet extends StatelessWidget {
  const _GlassMenuSheet({required this.currentPath});
              ...
              child: LiquidGlassSection(
                ...
                children: [
                  LiquidGlassListTile(
                    ...
                    showChevron: true,
                    onTap: () =>
                        Navigator.of(context).pop(StudentRoutes.profile),
                  ),
                ],
              ),
            ),
  ...
  bool _isActive(StudentNavItem item) =>
