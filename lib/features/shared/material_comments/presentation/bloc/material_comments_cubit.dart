...
class MaterialCommentsCubit extends Cubit<RemoteState<List<MaterialComment>>> {
  ...
  /// The section whose thread this is. Required when reading as a teacher,
  /// null as a student — the student endpoint derives it from the caller.
  ...
  Future<void> load({bool refresh = false}) async {
    ...
    final result = await _list(
      CommentThreadParams(materialId: materialId, divisionId: divisionId),
    );
    if (isClosed) return;
