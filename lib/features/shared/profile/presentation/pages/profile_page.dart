...
import '../../../shell/presentation/widgets/nav_spec.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return ShellScaffold(
      child: ListView(
        ...
        children: [
          _ProfileHeader(
            // The screen serves every role area, so the placeholder shown while
            // the session resolves names the active role rather than assuming.
            name: user?.displayName ?? state.activeRole?.label ?? 'Account',
            email: user?.email,
            avatar: user?.avatar,
          ...
          LiquidGlassSection(
            children: [
              LiquidGlassListTile(
                ...
                showChevron: true,
                // The settings screen of *this* area. Hardcoding the student
                // path would send a teacher to `/student/settings`, which the
                // router's role guard bounces straight back to their home.
                onTap: () => context.go(
                  NavSpecScope.maybeOf(context)?.settingsPath ??
